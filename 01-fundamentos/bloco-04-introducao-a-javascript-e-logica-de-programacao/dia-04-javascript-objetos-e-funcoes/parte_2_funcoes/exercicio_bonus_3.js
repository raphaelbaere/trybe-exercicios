...
const basket = [
  ...
  function contaCesta(basket) {
    let objetoCesta = {};
    let contNumero = 0;
    for (let index=0;index < basket.length;index+=1) {
        let verificaNumero = basket[index]
        for (let index2=0;index2 < basket.lenth;index2+=1) {
            if (!objetoCesta[verificaNumero]) objetoCesta[verificaNumero] = 0;
            objetoCesta[verificaNumero] += 1;
        }
        if(basket[index] !== objetoCesta[basket[index]]) {
            objetoCesta[basket[index]] = contNumero;
        }
        contNumero = 0;
    }
    return objetoCesta
